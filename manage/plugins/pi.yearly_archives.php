...
class Yearly_archives {
	...
	function Yearly_archives()
	{
		...
		$sql_expired = ($TMPL->fetch_param('show_expired') == 'yes')		? "" : "AND ( expiration_date = 0 OR expiration_date > UNIX_TIMESTAMP() )";
		$sql_future	 = ($TMPL->fetch_param('show_future_entries') == 'yes')	? "" : "AND entry_date < UNIX_TIMESTAMP()";
		...
		$sql = "
			SELECT
				FROM_UNIXTIME( entry_date, '%Y%m' ) AS ym,
