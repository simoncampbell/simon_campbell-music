	...
	*/
	function show_full_control_panel_end( $out )
	{
		...
		if($this->settings['enable'] == 'y')
		{
			...
			// html pattern
			$patterns[0] = "/(<div class='helpLinksLeft' >)/";

			// style pattern
			$patterns[1] = "/<\/style/";

			// html replacement
			$replacements[0] = "<div class='helpLinksLeft' >" . $xhtml;

			// style replacement
			$replacements[1] = $this->settings['css']."</style";
			
			if(isset($this->settings['enable_page_title_replacement']) === TRUE && $this->settings['enable_page_title_replacement'] == 'y')
			{
				$patterns[2] = "/ExpressionEngine<\/title>/";
				$replacements[2] = str_replace("{site_name}", $PREFS->core_ini['site_name'], $this->settings['page_title_replacement_value']) . "</title>";
			}
