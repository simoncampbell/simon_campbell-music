...
{
	require_once APPPATH.'third_party/bridge/lib/addon_builder/module_builder.php';
}
...
class User extends Module_builder_bridge
{
	...
	protected $cat_formatting	= array( 'category_tagdata' => '', 'category_formatting' => '', 'category_header' => '', 'category_indent' => '', 'category_body' => '', 'category_footer' => '', 'category_selected' => '', 'category_group_header' => '', 'category_group_footer' => '' );
	protected $standard			= array( 'url', 'location', 'occupation', 'interests', 'language', 'last_activity', 'bday_d', 'bday_m', 'bday_y', 'daylight_savings', 'aol_im', 'yahoo_im', 'msn_im', 'icq', 'bio', 'profile_views', 'time_format', 'timezone', 'signature' );
	protected $check_boxes		= array( 'accept_admin_email', 'accept_user_email', 'daylight_savings', 'notify_by_default', 'notify_of_pm', 'smart_notifications' );
	protected $photo			= array( 'photo_filename', 'photo_width', 'photo_height' );
	protected $avatar			= array( 'avatar_filename', 'avatar_width', 'avatar_height' );
	protected $signature		= array( 'signature', 'sig_img_filename', 'sig_img_width', 'sig_img_height' );
	protected $images			= array( 'avatar' => 'avatar_filename', 'photo' => 'photo_filename', 'sig' => 'sig_img_filename' );
	...
	public function __construct()
	{
        ...
        /** --------------------------------------------*/
     	...
     	if (ee()->config->item('req_mbr_activation') == 'manual' && ee()->db->table_exists('exp_user_welcome_email_list'))
        {
        	$query = ee()->db->query("SELECT m.screen_name, m.email, m.username, m.member_id 
    ...
    {	
		...
		$exclude	= array( 'channel_id', 'weblog_id', 'tmpl_group_id', 'upload_id', 'password', 'unique_id', 
							 'authcode', 'avatar_filename', 'avatar_width', 'avatar_height', 
							 'photo_width', 'photo_height', 'sig_img_width', 'sig_img_height', 
							 ...
							 'cp_theme', 'profile_theme', 'forum_theme', 'tracker', 'notepad', 
							 'notepad_size', 'quick_links', 'quick_tabs', 'pmember_id' );
		...
		$query	= ee()->db->query("SELECT m.*, mg.group_title,
		...
		$this->member_only	= ( ee()->TMPL->fetch_param('member_only') !== FALSE AND ee()->TMPL->fetch_param('member_only') == 'no' ) ? FALSE: TRUE;
		...
		if ($query->row('bio') != '')
		{
			$query_row['bio'] = ee()->typography->parse_type($query->row('bio'), 
														 array(
																	'text_format'   => 'xhtml',
																	'html_format'   => 'safe',
																	'auto_links'    => 'y',
		...
		{					
			if (preg_match_all("/".LD.$val."\s+format=([\"'])([^\\1]*?)\\1".RD."/s", $tagdata, $matches))
			{
				for($i=0, $s=sizeof($matches[2]); $i < $s; ++$i)
				{
					...
					{
						$str	= str_replace( $code, ee()->localize->convert_timestamp( $code, $query_row[$val], TRUE ), $str );
					}
	...
	public function _users( $inject = array() )
    {
		...
		if ( ee()->TMPL->fetch_param('orderby') !== FALSE AND ee()->TMPL->fetch_param('orderby') == "principal" )
		{
		...
		foreach ( $this->query->result_array() as $count => $row )
		{
			...
			$row['total_combined_posts'] = $row['total_forum_topics']+$row['total_forum_posts']+$row['total_entries']+$row['total_comments'];
			...
			{
				$row['bio'] = ee()->typography->parse_type(	$row['bio'], 
														 array(
																	'text_format'   => 'xhtml',
																	'html_format'   => 'safe',
																	'auto_links'    => 'y',
	...
	public function authors()
    {
		...
		$sql	= "SELECT ua.author_id, ua.principal FROM exp_user_authors ua 
	...
	{	
		...
		if ( APP_VER < 2.0 && class_exists('Weblog') === FALSE )
        {
	...
	public function edit()
    {
		...
		foreach ( ee()->TMPL->var_pair as $key => $val )
		{
			...
			if ( $key == 'languages' )
			{
				...
				if ($fp = @opendir(PATH_LANG))
				{
					while (FALSE !== ($file = readdir($fp)))
					{
						if (is_dir(PATH_LANG.$file) && substr($file, 0, 1) != ".")
						{
							$dirs[] = $file;
						}
					}
	...
	public function edit_profile()
    {
        ...
        if ($admin === FALSE)
        {
        	$check = array('username', 'screen_name', 'email');
	...
	public function reg( $remote = FALSE )
    {
       ...
       /** --------------------------------------------*/
		...
		{
			$errors = ee()->extensions->universal_call_extension('user_register_error_checking', $this, $errors);
			if (ee()->extensions->end_script === TRUE) return;
		}
     	...
     	if (ee()->config->item('req_mbr_activation') == 'manual')
        {
			...
			ee()->db->query(ee()->db->insert_string('exp_user_welcome_email_list',
													array(	'member_id' => $member_id, 
	...
	public function do_search()
    {
        ...
        foreach ( $this->standard as $field )
        {
        	if ( isset( $_POST[ $field ]) && ! in_array( $field, $skip ))
        	{
        		$this->insert_data[$field]	= $_POST[ $field ];
        	}
        }
	...
	public function key()
    {
		...
		if ( preg_match( "/".LD.'member_groups'.RD."(.*?)".LD.preg_quote(T_SLASH, '/').'member_groups'.RD."/s", $tagdata, $match ) )
		{
			$query	= ee()->db->query( "SELECT DISTINCT group_id, group_title FROM exp_member_groups WHERE group_id NOT IN (1,2,3,4)" );
		...
		$this->form_data['id']					= ( ee()->TMPL->fetch_param('form_id') ) ? ee()->TMPL->fetch_param('form_id'): 'member_form';
		
		$this->params['template']			= ( ee()->TMPL->fetch_param('template') ) ? ee()->TMPL->fetch_param('template'): '';
		
		$this->params['html']				= ( ee()->TMPL->fetch_param('html') == 'yes' ) ? 'yes': 'no';
		
		$this->params['word_wrap']			= ( ee()->TMPL->fetch_param('word_wrap') != 'yes' ) ? 'no': 'yes';
		
		$this->params['parse']				= ( ee()->TMPL->fetch_param('parse') !== FALSE && in_array(ee()->TMPL->fetch_param('parse'), array('br', 'none', 'xhtml'))) ? ee()->TMPL->fetch_param('parse'): 'none';
		
		$this->params['return']				= ( ee()->TMPL->fetch_param('return') ) ? ee()->TMPL->fetch_param('return'): '';
    ...
    {   
		...
		$vars['from_email']	= ( ee()->input->get_post('from') !== FALSE ) ? ee()->input->get_post('from'): ee()->config->item('webmaster_email');
		$vars['from_name']	= ( ee()->input->get_post('name') !== FALSE ) ? ee()->input->get_post('name'): ee()->config->item('webmaster_name');
		$vars['subject']	= ( ee()->input->get_post('subject') !== FALSE ) ? ee()->input->get_post('subject'): 'You are invited to join '.ee()->config->item('site_name');
		$vars['message']	= ( ee()->input->get_post('message') !== FALSE ) ? ee()->input->get_post('message'): '';
		...
		{					
			...
			$query = ee()->db->query("SELECT t.template_type, t.template_data 
										   FROM exp_templates AS t 
										   JOIN exp_template_groups AS tg ON tg.group_id = t.group_id 
			...
			if ( $this->_param('parse') !== FALSE && in_array($this->_param('parse'), array('br', 'none', 'xhtml')))
			{
				$formatting['text_format'] = $this->_param('parse');
			}
			
			$body = ee()->typography->parse_type(
									stripslashes( ee()->security->xss_clean( ee()->template->final_template ) ), $formatting
									);
		}
		...
		if ( ee()->input->get_post('to') )
		{
			$email	= explode( ",", ee()->input->get_post('to') );
    ...
    {	
    	$query = ee()->db->query("SELECT preference_value FROM exp_user_preferences WHERE preference_name = 'screen_name_override' LIMIT 1");
    	...
    	if ( ! empty($screen_name_override) )
    	{
        	...
        	if ( $fields != '' )
        	{
				return $this->_output_error( 'general', array( str_replace( "%s", $fields, ee()->lang->line('user_field_required') ) ) );
        	}
			...
			{
				return $this->_output_error( 'general', array( str_replace( "%s", $screen_name, ee()->lang->line('banned_screen_name') ) ) );
			}
    ...
    {	
		...
		if ( $type == 'update' AND ( ! ee()->input->get_post('username') OR ee()->input->get_post('username') == $cur_username ) )
		{
			return TRUE;
		}
		...
		if ( ee()->session->ban_check( 'username', ee()->input->get_post('username') ) OR ee()->session->ban_check( 'email', ee()->input->get_post('username') ) )
		{
		...
		if ($this->preferences['email_is_username'] != 'n' && ($key = array_search(ee()->lang->line('username_password_too_long'), ee()->email_validate->errors)) !== FALSE)
		{
	...
	{	
		...
		if (APP_VER < 2.0)
		{
			...
			if ( ! is_object($this->UP))
			{
				$this->UP = new Upload();
			}
			
			/**	----------------------------------------
			/**	Force the userfile array since that's what the Upload class looks for.
			/**	----------------------------------------*/
			
			$_FILES['userfile']	= $_FILES[ $this->uploads[$type] ];
			...
			if ( ! $this->UP->upload_file())
			{
				...
				$info = ($this->UP->error_msg == 'invalid_filetype') ? "<div class='itempadbig'>".ee()->lang->line('invalid_image_type')."</div>" : '';
				return $this->_output_error('submission', ee()->lang->line($this->UP->error_msg).$info);
			}
			...
			if ($width > $max_width OR $height > $max_height)
			{
				...
				if ( ! $this->_image_resize($new_filename, $type))
				{
					...
					$max_size = str_replace('%x', $max_width, ee()->lang->line('max_image_size'));
					$max_size = str_replace('%y', $max_height, $max_size);
					$max_size .= ' - '.$max_kb.'KB';
	
					return $this->_output_error('submission', $max_size);
				}
			}
			...
			// Since our image resizing class will only re-proportion
			// based on one axis, we'll check the size again, just to 
			// be safe.  We need to make absolutely sure that if someone
			// submits a very short/wide image it'll constrain properly
		...
		else
		{
			...
			$config['file_name']		= str_replace(strtolower($extension), '', $new_filename);
			...
			if ($this->EE->upload->do_upload($this->uploads[$type]) === FALSE)
			{
	...
	{	
		...
		if ($type == 'avatar')
		{
			$max_width	= (ee()->config->slash_item('avatar_max_width') == '' OR ee()->config->item('avatar_max_width') == 0) ? 100 : ee()->config->item('avatar_max_width');
			$max_height	= (ee()->config->item('avatar_max_height') == '' OR ee()->config->item('avatar_max_height') == 0) ? 100 : ee()->config->item('avatar_max_height');	
			$image_path = ee()->config->item('avatar_path').'uploads/';
		}
		elseif ($type == 'photo')
		{
			$max_width	= (ee()->config->slash_item('photo_max_width') == '' OR ee()->config->item('photo_max_width') == 0) ? 100 : ee()->config->item('photo_max_width');
			$max_height	= (ee()->config->item('photo_max_height') == '' OR ee()->config->item('photo_max_height') == 0) ? 100 : ee()->config->item('photo_max_height');	
			$image_path = ee()->config->item('photo_path');		
		}
		else
		{
			$max_width	= (ee()->config->slash_item('sig_img_max_width') == '' OR ee()->config->item('sig_img_max_width') == 0) ? 100 : ee()->config->item('sig_img_max_width');
			$max_height	= (ee()->config->item('sig_img_max_height') == '' OR ee()->config->item('sig_img_max_height') == 0) ? 100 : ee()->config->item('sig_img_max_height');	
		...
		$res = $IM->set_properties(			
									array(
											'resize_protocol'	=> ee()->config->item('image_resize_protocol'),
											'libpath'			=> ee()->config->item('image_library_path'),
											'maintain_ratio'	=> TRUE,
											'master_dim'		=> $axis,
