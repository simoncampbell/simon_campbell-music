 ...
 * @package 	Solspace:User
 * @author		Solspace DevTeam
 * @filesource 	./system/modules/user/parser.user.php
 */
...
if (APP_VER < 2.0)
{
	...
	eval('class User_parser_base extends Template { }');
}
else
{
	require_once BASEPATH.'expressionengine/third_party/bridge/lib/addon_builder/addon_builder.php';
	eval('class User_parser_base extends EE_Template { }');
}

class User_parser extends User_parser_base
{ 
	// --------------------------------------------------------------------
    ...
    public function fetch_modules()
    {
        ...
        if ( isset(ee()->session->cache['modules']['user']['template']['fetch_modules']))
        {
        	$this->modules = ee()->session->cache['modules']['user']['template']['fetch_modules'];
        	return;
        }
		...
		$this->modules = ee()->session->cache['modules']['user']['template']['fetch_modules']= array_unique($this->modules);
    }
    /* END fetch_modules() */
    ...
    public function fetch_plugins()
    {
        ...
        if ( isset(ee()->session->cache['modules']['user']['template']['fetch_plugins']))
        {
        	$this->plugins = ee()->session->cache['modules']['user']['template']['fetch_plugins'];
        	return;
        }
		...
		$this->plugins = ee()->session->cache['modules']['user']['template']['fetch_plugins'] = array_unique($this->plugins);
    }
    /* END fetch_plugins() */
