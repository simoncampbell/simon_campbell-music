...
{   
    ...
    public function __construct( $switch = TRUE )
    {
        ...
        $menu	= array('module_reassign_ownership'	=> array(	'link'  => $this->base.'&method=reassign_ownership',
        														'title' => ee()->lang->line('reassign_ownership')),
        													
						'module_preferences'		=> array(	'link'  => $this->base.'&method=preferences',
        														'title' => ee()->lang->line('preferences')),
        														
        				'module_documentation'		=> array(	'link'  => USER_DOCS_URL,
        														'title' => ee()->lang->line('documentation').
        																	'&nbsp;(<span style="font-size:11px">v.'.USER_VERSION.'</span>)'),
        				);
	...
	public function preferences( $message = '' )
    {
		...
		$query = ee()->db->query("SELECT * FROM exp_user_preferences");
    ...
    public function update_preferences( $message = '' )
    {
		...
		foreach($prefs as $pref)
		{
			if ( isset( $_POST[$pref] ) )
			{
				ee()->db->query("DELETE FROM exp_user_preferences WHERE preference_name = '".ee()->db->escape_str($pref)."'");
				
				ee()->db->query(ee()->db->insert_string('exp_user_preferences', 
																  array('preference_value' => $_POST[$pref],
