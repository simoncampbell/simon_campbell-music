...
class Low_variables_type {
	...
	*/
	function __construct($get_language_file = TRUE)
	{
		...
		{
			// get current global debug value
			// turn it off temporarily, while we fetch
			// the language file for this type, looking in the package
			$ini_debug = $PREFS->ini('debug');

			if ($ini_debug) $PREFS->core_ini['debug'] = FALSE;		// -- off
			$LANG->fetch_language_file($this->type, 'low_variables');
			if ($ini_debug) $PREFS->core_ini['debug'] = $ini_debug; // and on again
			...
			if (isset($this->language_files) && is_array($this->language_files))
			{
				...
				{
					$LANG->fetch_language_file($file, 'low_variables');
				}
			}
		}
		...
		if (isset($this->info['name']) && ($name = $LANG->line($this->type.'_type_name')))
		{
			$this->info['name'] = $name;
		}
	}
	...
	*/
	function load_assets()
	{
		...
		$this->asset_url = $PREFS->ini('theme_folder_url') . "third_party/low_variables/types/{$this->type}/";
		...
		if (isset($this->assets['css']))
		{
			...
			foreach ($css AS $file)
			{
				$DSP->extra_header .= "\n\t".'<link rel="stylesheet" type="text/css" href="'.$this->asset_url.$file.'" />';
			}
		}
		...
		if (isset($this->assets['js']))
		{
			...
			foreach ($js AS $file)
			{
				$DSP->extra_header .= "\n\t".'<script type="text/javascript" src="'.$this->asset_url.$file.'"></script>';
			}
		}
	...
	*/
	function insert_js($js)
	{
		...
		$DSP->extra_header .= NL.'<script type="text/javascript">'.NL.$js.NL.'</script>'.NL;
	}
	...
	function separator_select($selected = 'newline')
	{
		...
		foreach ($this->separators AS $sep => $char)
		{
			$r .= '<label class="low-radio">'
				. 	$DSP->input_radio($this->input_name('separator'), $sep, ($sep == $selected))
				. 	$LANG->line($sep)
				. '</label>';
		}
	...
	*/
	function interface_select($selected = 'select')
	{
		...
		{
			$options[$interface] = $LANG->line($interface);
		}
	...
	*/
	function select_element($var_id, $rows, $selected = array(), $multiple = FALSE, $none = TRUE)
	{
		...
		else
		{
			...
			if ($none)
			{
				$rows = array('' => '-- '.$LANG->line('none').' --') + $rows;
			}
